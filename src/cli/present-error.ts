 ...
 * {@link Error#message} on tagged errors (that string stays diagnostic). Low-level
 * OS / platform text (e.g. `SystemError#description`) is not shown here; use
 * {@link formatCliFailureDiagnostics} when `--debug` or `DRY_AI_DEBUG` is on.
 */
...
/**
 * Verbose Effect cause tree for stderr when diagnostics mode is enabled
 * (`--debug` or `DRY_AI_DEBUG`).
 */
export function formatCliFailureDiagnostics(
  cause: Cause.Cause<unknown>,
): string {
